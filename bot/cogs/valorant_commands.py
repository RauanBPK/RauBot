...
from bot.utils import agents, get_id_from_mention, hypes, insults, mapas, nl
...
class Valorant(commands.Cog):
    ...
    @commands.command(name="mapa", help="Sorteia um mapa para ser jogado")
    async def random_map(self, ctx):
        mapa = random.choice(mapas)
        await ctx.send(f"O mapa sorteado foi **{mapa}**! 🤪")

    @commands.command(name="boneco", help="Escolha um boneco pra jogar. <role> opcional")
    async def random_char(self, ctx, role=None):
        try:
    ...
    @commands.command(name="role", help="Escolha um role")
    ...
    @commands.command(name="times", help="!times @<user1> @<user2>... para formar times ")
    async def random_teams(self, ctx, *member_mentions):
        try:
            ...
            team1_names = [await self.bot.fetch_user(user_id) for user_id in team1 if isinstance(user_id, int)]
            ...
            team2_names = [await self.bot.fetch_user(user_id) for user_id in team2 if isinstance(user_id, int)]
            ...
            await ctx.send(
                f"Boa sorte pros **{insult.lower()}s** do Time{1 if vai_ganhar == 2 else 2},"
    ...
    @commands.command(
        name="mover",
        help="!mover <canal_de_voz> @<user1> @<user2> para mover os times para outro canal de voz",
    )
    async def move_to_voice_channel(self, ctx, to_channel: str, *member_mentions):
        try:
            ...
            if ctx.guild:
                ...
                if to_channel:
                    ...
                    for member_mention in member_mentions:
                        try:
                            member_id = int(
                                member_mention.replace("<@", "").replace(">", "")
    ...
    @commands.command(name="toprajogo", help="!toprajogo - !toprajogo reset || lista || remove")
    async def toprajogo(self, ctx, command=None):
    ...
    @commands.command(name="5v5", help="!5v5 - !5v5 reset || lista || remove || juntar")
    async def five_vs_five(self, ctx, command=None):
        ...
        if ctx.author.id not in [user.id for user in players_queue]:
            ...
            if len(players_queue) == 10:
                ...
                hype = random.choice(hypes)
                await ctx.send(
                    f"Boa sorte pros **{insult.lower()}s** do Time{1 if vai_ganhar == 2 else 2}, o Time{vai_ganhar} é **muito** {hype}! 😏"
                )
        else:
            insult = random.choice(insults)
            await ctx.send(f"{ctx.author.mention}, você já está na lista **{insult}** 🙄")

    @commands.command(name="comp", help="!comp <mapa> || random")
    async def comp_maker(self, ctx, command=None):
        ...
        if not command:
            ...
            try:
                ...
                await ctx.send(
                    f"Frequência: **{pick_rate}%**{nl}Vezes utilizada: **{times_played}**{nl}"
